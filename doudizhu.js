...
var doudizhu = function () {
    ...
    this.getCardType = function (arr) {
        ...
        // 将牌数组的id装换成 单张 牌对应的牌权重，并通过 字典 存储该权重牌的张数
        arr.forEach(function (card, index) {
            var cardWeight = this.cardInfo.getWeight(card)// 权重
        ...
        for (var card in dictionaryItems) {
            if (dictionaryItems[card] > cardMaxNum) {
                cardMaxNum = dictionaryItems[card]//牌数
                muchCardWeight = card
            }
        }
        switch (cardMaxNum) {
